...
class legImitationEnvV0(BaseV0):
    ...
    def _setup(self,
               ...
               ref_cand = 'cand.pkl',
               ref_path = '/',
               ref_sample_type = 0, # 0 for random, 1 for sequential
               **kwargs,
               ):
        ...
        self.ref_cand = joblib.load(os.path.join(ref_path, ref_cand))
        self.ref_sample_type = ref_sample_type
        if self.ref_sample_type == 0: # random
            self.refid = np.random.randint(len(self.ref_cand))
        else:
            self.refid = 0
        self.ref       = joblib.load(self.ref_cand[self.refid])
        self.init_qpos[:] = self.ref['qpos'][0][:]
        self.init_qvel[:] = self.ref['qvel'][0][:]
    ...
    def get_obs_dict(self, sim):
        ...
        obs_dict['ref_qpos'] = self.ref['qpos'][self.steps + 1][:].copy()
        obs_dict['ref_qvel'] = self.ref['qvel'][self.steps + 1][:].copy()
    ...
    def get_reward_dict(self, obs_dict):
        ...
        ref_qpos_rew = self.get_ref_qpos_rew()
        ref_qvel_rew = self.get_ref_qvel_rew()
        ...
        rwd_dict = collections.OrderedDict((
            ...
            # Must keys
            # ('sparse', vel_reward),
            ('solved', self.steps >= len(self.ref['qpos']) - 2), # TODO: a better condition
            ('done',   self._get_done()),
        ))
    ...
    def get_ref_qpos_rew(self, obs_dict):
        return np.abs(sim.data.qpos[:][3:] - obs_dict['ref_qpos'][3:]).mean()
        
    def get_ref_qvel_rew(self, obs_dict):
        return np.abs(sim.data.qvel[:][3:] - obs_dict['ref_qvel'][3:]).mean()
    ...
    def reset(self, **kwargs):
        ...
        self.refid = min(self.refid + 1, len(self.ref_cand) - 1)
    ...
    def _get_done(self):
        ...
        if height < self.min_height:
            return 1
        if self.steps >= len(self.ref['qpos']) - 2):
            return 1
        return 0
